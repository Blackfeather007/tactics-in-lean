...
example (S T : Set ℝ) (x : ℝ) (h : x ∈ S ∩ T) : x ∈ S := by
  have ⟨h_left, h_right⟩ := h  -- Use pattern matching to destructure the definition of intersection
  exact h_left
...
example (a b : ℝ) (h : (a, b) = (1, 2)) : a = 1 := by
  have ⟨h1, h2⟩ : a = 1 ∧ b = 2 := by
    exact Prod.mk.inj_iff.mp h
  exact h1
